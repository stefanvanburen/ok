...
func TestAssertions(t *testing.T) {
	...
	tests := []struct {
		...
		{"True pass with message", func(tb ok.TB) bool { return ok.True(tb, 2 < 3, ok.Sprintf("got %d, want < %d", 2, 3)) }, nil},
		{"True fail with message", func(tb ok.TB) bool { return ok.True(tb, 2 > 3, ok.Sprintf("got %d, want > %d", 2, 3)) }, []string{"got false, want true: got 2, want > 3"}},
